...
- **OpenIV XML dialect is tolerated.** Most packages are authored against OpenIV, which spells some things differently — an empty array is `<Variations type="NULL"/>` there and `<Variations/>` here. Holding a package to CodeWalker's spelling made the check above reject good edits (adding a vehicle to `popgroups.ymt`, for one). Rather than hardcode which attributes are cosmetic — `type` is meaningful on that very element when the array *isn't* empty — the installer proves it: it strips the disputed attribute, rebuilds, and only accepts if the output is byte-identical. It also has to sit on an element with no children and no text. Anything else is still refused, and every allowance is logged.
